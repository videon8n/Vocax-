...
export default function ResultadoPage() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        const colors = ['#F5A65B', '#E0457B', '#7BCFA8'];
        confetti({
          particleCount: 80,
          spread: 70,
          origin: { y: 0.35 },
        ...
        // ignore — confetti é opcional
      }
      playReveal({ enabled: soundEnabled });
    }, 380);
    return () => clearTimeout(t);
  }, [profile, revealed, reducedMotion, soundEnabled]);
  ...
  const stagger = {
    ...
    transition: { duration: reducedMotion ? 0 : 0.45, ease: [0.22, 1, 0.36, 1] as [number, number, number, number] },
  };
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-5xl px-6 py-10 md:py-14">
        <motion.p
          {...stagger}
          className="text-sm uppercase tracking-[0.2em] text-amber mb-4"
        >
          Passo 3 de 3 · Resultado
        </motion.p>
        <motion.h1
          initial={stagger.initial}
          animate={stagger.animate}
          transition={{ ...stagger.transition, delay: reducedMotion ? 0 : 0.05 }}
        ...
        <div className="mt-12 grid lg:grid-cols-[360px_1fr] gap-10 items-start">
          ...
          <motion.div
            ...
            transition={{ duration: reducedMotion ? 0 : 0.6, delay: reducedMotion ? 0 : 0.15, ease: [0.22, 1, 0.36, 1] }}
          >
          ...
          <div className="space-y-8">
              ...
              <Section title="Seu timbre" subtitle="Como sua voz é descrita por quem te escuta.">
                <div className="flex flex-wrap gap-2 mt-3">
                  {timbre.adjectives.map((adj) => (
                    <span
                      key={adj}
                      className="rounded-full bg-vocax-gradient px-4 py-1.5 text-sm font-medium text-white shadow-glow-magenta"
                    >
                      {adj}
                    </span>
                  ))}
            ...
            >
              <div className="flex-1">
                ...
                <p className="mt-2 text-graphite-200">
                  Escolhemos faixas que cabem perfeitamente na sua voz.
                </p>
              </div>
            ...
            <div className="text-sm text-graphite-300 flex flex-wrap items-center gap-3 pt-4">
              <span>
                Análise gerada em{' '}
              ...
              <button
                ...
                className="inline-flex items-center gap-1 text-graphite-200 hover:text-graphite-50 underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber rounded"
              >
